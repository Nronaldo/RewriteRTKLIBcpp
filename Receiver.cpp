...
receiver::receiver(NAV_RINEX &nav_rinex,int dynamic_on,int arfilterpara,string fname) {
	...
	excsatindex = 0;
	nfix = 0;
	sol = solution(2);//强行设置成2系统
	string filename = "../solfile/" + fname + ".txt";
...
void receiver::updateobs(const OBS_RINEX & obsrinex) {
	...
	distributesats(obsrinex.GPS_satID_B, obsrinex.GPS_satID_R, GPSobs, 'G', obsrinex);
	distributesats(obsrinex.BDS_satID_B, obsrinex.BDS_satID_R, BDSobs, 'B', obsrinex);
	
}
void receiver::distributesats(const vector<int> &GNSS_ID_B, const vector<int> &GNSS_ID_R,map<int, OBS_SAT> &mp,char mode, const OBS_RINEX & obsrinex) {
	mp.clear();
	for (int i = 0; i < GNSS_ID_B.size(); i++) {
...
void receiver::choosecommonsat() {
	...
	for (; itg != GPSobs.end(); ) {
		...
		itg++;
		if (!it->second.get_baseflag() || !it->second.get_roverflag())
			GPSobs.erase(it);
	}
	for (; itb != BDSobs.end();) {
...
void receiver::soltofile(string mode,int LS_stat) {
...
int  receiver::LS_pos_body(char mode, int countG, int countB) {
	...
	if (delta_x.norm() < 1E-4 && (mode == 'N'||(mode =='F'&&Ls_res<Ls_res_threshold))) {
...
int receiver::RTK_pos(string RTKmode)
{
	...
	if (RTKmode == "PL") {
		...
		if (!solve_validpos(RTKmode,NULL)) {
			/*if (numiterator >= 147)
				return 0;*/
			cout << "有效卫星数太少" << endl;
		...
		if ((nb = manage_amb_LAMBDA(RTK_xfix,ix)) > 1) {
			...
			if (solve_validpos(RTKmode, RTK_xfix)) {
				if (++nfix >= minfix) {
				//if (++nfix >= 1) {
					/*if (numiterator == debug_stop) {
						int aaaa = 1;
					}
					holdamb(RTK_xfix,ix,nb, firstvalidGPSID, firstvalidBDSID);//先把hold模式禁用了
					if (numiterator == debug_stop) {
						int aaaa = 1;
					}*/
				}
...
void receiver::lock_solve() {
	...
	for (; itg != GPSobs.end(); itg++) {
		if (itg->second.get_BR_fdemask() || itg->second.get_BR_unusedflag())
			continue;
		if (lock_G[itg->first - 1] < 0 || (nfix > 0 && itg->second.get_fix() >= 2))
			lock_G[itg->first - 1]++;
	...
	for (; itb != BDSobs.end(); itb++) {
		if (itb->second.get_BR_fdemask() || itb->second.get_BR_unusedflag())
			continue;
		if (lock_B[itb->first - 1] < 0 || (nfix > 0 && itb->second.get_fix() >= 2))
			lock_B[itb->first - 1]++;
	}
}
...
void receiver::calddres(string RTKmode,double * state_x) {
	...
	if (RTKmode == "PL") {
		...
		for (; itg != GPSobs.end()&& SATrefGPSID; itg++) {
			...
			if (fabs(pseudres) > optmaxino && fabs(carrierdres) > optmaxino)
				continue;
			if (fabs(pseudres) <= optmaxino) {
				int mapkey = itg->first;
				SatSat_PdresG.insert(make_pair(mapkey, pseudres));
			}
			if (fabs(carrierdres) <= optmaxino) {
				int mapkey = itg->first;
...
void receiver::initialize_bias_state() {
	...
	if (j > 0) {
		...
		for (itb = BDSobs.begin(); itb != BDSobs.end(); itb++) {
			if (numiterator >= debug_stop && itb->first == 8) {
				int aaa = 1;
			}
			if (itb->second.get_BR_unusedflag())
				continue;
	...
	for (itg = GPSobs.begin(); itg != GPSobs.end(); itg++) {
		...
		int xii = index - 9+1;*/
		initxP(bias_G[itg->first], (optstd[0]* optstd[0]), index);
		//lock_G[itg->first - 1] = -minlock;
	}
	for (itb = BDSobs.begin(); itb != BDSobs.end(); itb++) {
		if (itb->second.get_BR_unusedflag())
		...
		double lambdab, lambdar;
		itb->second.get_lambda(0, &lambdab, &lambdar);
		int xii = index - 9 + 1 - NSATGPS;
		double xxxx = (bias_B[itb->first] - com_bias) / lambdar;
		initxP(bias_B[itb->first], (optstd[0] * optstd[0]), index);
		//lock_B[itb->first - 1] = -minlock;
	}
...
void receiver::udpos() {
	...
	if (norm_array(RTK_x, 3) <= 0.0) {
		for (int i = 0; i < 3; i++) {
			initxP(sol.getpos()[i],VAR_POS,i);
		}
		if (dynamics_on) {
			for (int i = 3; i < 6; i++) {
				initxP(sol.getvel()[i-3], VAR_VEL, i);
			}
			for (int i = 6; i < 9; i++) {
				initxP(1E-6, VAR_ACC, i);
			}
		}
	}
	...
	Q(0, 0) = Q(1, 1) = pow(prn[3], 2)*fabs(tt);
	Q(2, 2) = pow(prn[4], 2)*fabs(tt);
...
int receiver::solve_validpos(string RTKmode,double * state) {//RTKLIB 在做完卡尔曼滤波后先要对残余进行一次判定，但并不影响值，只是提示。同时会对有效卫星的outc进行清零操作
	...
	if (valpos(thres)) {
		...
		if (nv_dim / 2 + (SATrefGPSID ? 1 : 0) + (SATrefBDSID ? 1 : 0) < 4) {//能产生双差，且双差残余比较小的
			return 0;
		}
		return 1;
...
int receiver::manage_amb_LAMBDA(double *rtk_fixX,vector<int> & ix) {
	...
	if (arfilter) {
		rerun = 0;
		if (nb >= 0 && ((sol.get_ratio() < optthresar[0] * 1.1 && sol.get_ratio() < sol.get_prev_ratio1() / 2.0) || // 一般情况下，prev_ratio1 和 prev_ratio2 都会设置成
...
int receiver::exclude_different_sat(int &exc_satsID,int &exc_sats_lock) {
	...
	if (sol.get_prev_ratio2() > sol.get_thres() || valid_num < optmindropsats) {
		exc_satsID = 0;
		exc_sats_lock = 0;
		return 0;
	}
...
int receiver::exclude_new_sat() {
	...
	for (; itg != GPSobs.end(); itg++) {
		...
		if (lock_G[itg->first - 1] == 0) {
			lock_G[itg->first - 1] = -minlock - dly;
			dly += 2;
			rerun = 1;
		}
	}
	for (; itb != BDSobs.end(); itb++) {
		...
		if (lock_B[itb->first - 1] == 0) {
			lock_B[itb->first - 1] = -minlock - dly;
			dly += 2;
			rerun = 1;
		}
	}
...
int receiver::resamb_LAMBDA(double * RTK_fixX,vector<int > & ix) {
	...
	if ((nb = ddmat_FormxP(D, x, P, ix)) < optminfixsats - 1) {
...
int receiver::ddmat_FormxP(Eigen::MatrixXd & D, Eigen::VectorXd &x, Eigen::MatrixXd & P, vector<int> &ix) {
	...
	if (GPSfixddresnum + BDSfixddresnum < optminfixsats - 1)
...
void receiver::restamb(double *RTK_fixX,const vector<int> & ix,  const Eigen::MatrixXd &b) {
	int na = dynamics_on ? 9 : 3;
	for (int i = 0; i < nx; i++)
...
void receiver::holdamb(double *RTK_xfix, const vector<int > &ddres_stateindex, const int nb,int firstvalidGPSID,int firstvalidBDSID) {
	...
	Eigen::MatrixXd H = Eigen::MatrixXd::Zero(nb, all_statenum);
	Eigen::VectorXd x = Eigen::VectorXd::Zero(all_statenum);
	//Eigen::MatrixXd H_ = Eigen::MatrixXd::Zero(nb, pvanum+phasestatenum);
	//Eigen::VectorXd x_ = Eigen::VectorXd::Zero(all_statenum);
	Eigen::MatrixXd R= Eigen::MatrixXd::Zero(nb, nb);
	Eigen::MatrixXd P= Eigen::MatrixXd::Zero(all_statenum, all_statenum);
	...
	P = (I - K * H)*P;
	for (int i = 0; i < all_statenum; i++) {
		RTK_x[all_stateindex[i]] = x[i];
		for (int j = 0; j < all_statenum; j++) {
			RTK_P[all_stateindex[i] + all_stateindex[j] * nx] = P(i, j);
		}
	}
...
// 比较重要的是要筛选出指定间隔的数据。rover和base要同步。该间隔由全局变量tint指定
int screent_obs(ifstream &obsfileB, ifstream &obsfileR, string buffB, string buffR,receiver & rcv, OBS_RINEX &obs) {
	...
	// 另，当接收机某时刻数据的时候，不输出本时刻的时间戳，曾造成本函数陷入死循环的bug
	int Bupdateflag = 0;
