	...
	while (getline(obsfile, buff)) {
		if (buff[0] == '>') {
			...
			if (previousheadtime.time) {
				...
				if (timediffv > tint*1.3) {//手机有的时候会给出xxx.7299这样的时间，此处乘个系数是防止对这种文件算不出headtime
			...
			if (fmod(time2gpst(headtime, NULL) + DTTOL, tint) <= DTTOL * 2.0) { //找到的第一个时间必须是足够接近tint的时间才结束
...
void OBS_RINEX::readepoch(ifstream &fileB, ifstream &fileR, string buffB, string buffR,int &Bupdateflag,int &Rupdateflag, int &Btintflag, int &Rtintflag) {
	...
	gtime_t timestampB, timestampR;
		...
		if ((fabs(timediff(timestampB, timestampR)) <= 2 * DTTOL) || 
			(fabs(timediff(timestampB, timestampR)) > 2 * DTTOL && fmod(time2gpst(timestampB, NULL) + DTTOL, tint) > DTTOL * 2.0)||
			...
			if (fmod(time2gpst(timestampB, NULL) + DTTOL, tint) < DTTOL * 2.0) { //读取完以后时间满足了int指标，才应该置成1
				Btintflag = 1;
			}
			Bupdateflag = 1;
		}
		...
		if ((fabs(timediff(timestampB, timestampR)) <= 2 * DTTOL) ||
			(fabs(timediff(timestampB, timestampR)) > 2 * DTTOL && fmod(time2gpst(timestampR, NULL) + DTTOL, tint) > DTTOL * 2.0)||
			...
			if (fmod(time2gpst(timestampR, NULL) + DTTOL, tint) < DTTOL * 2.0) { //读取完以后时间满足了int指标，才应该置成1
				Rtintflag = 1;
			}
			Rupdateflag = 1;
		}
